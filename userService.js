...
module.exports.loginuserDBService = (employeeDetails)=> 
{
   return new Promise(function myFn(resolve, reject) 
   {
      userModel.findOne({ SDT: employeeDetails.SDT},function getresult(errorvalue, result)
      {
         if(errorvalue)
         {
         ...
         else
         {
            if(result !=undefined &&  result !=null)
            {
               ...
               {
                  resolve({status: true,msg: "Đăng nhập thành công"});
               }
               else
               {
                  reject({status: false,msg: "Đăng nhập thất bại"});
               }
            }
            else
            {
               reject({status: false,msg: "Tài khoản mật khẩu không có trong hệ thống"});
            }
